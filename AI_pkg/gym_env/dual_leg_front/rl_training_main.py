...
from gym_env.dual_leg_front import observation_cal
from gym_env.dual_leg_front import reward_cal
...
class DualLegFrontDogEnv(gym.Env):
    """
    ...
    def __init__(self, node : AIDogNode):
        ...
        self.observation_space = Discrete(169 * 2)
        ...
        self.targer_step = KeyboardAction.FORWARD_STEP_2.__len__() * 10
        self.__start_time = time.time()
        self.spot_type = "A" # Spot type 
                             # A: Spot move LF, RB
                             # B: Spot move RF, LB
    ...
    def step(self, action):
        ...
        if self.spot_type == "A" :
            new_moter_states = KeyboardAction.FORWARD_STEP_2[self.cnt % KeyboardAction.FORWARD_STEP_1.__len__()]
            new_moter_states[0] = self.first_motor_angle
            new_moter_states[6] = -self.first_motor_angle
        elif self.spot_type == "B":
            new_moter_states = KeyboardAction.FORWARD_STEP_3[self.cnt % KeyboardAction.FORWARD_STEP_1.__len__()]
        ...
        observation, state = observation_cal.get_observation(self.__node, spot_type=self.spot_type)
        ...
        if self.cnt == self.targer_step:
            ...
            if (angle > 15) and self.cnt > 0:
                ...
                reward -= 10000 * (1 - (self.cnt / self.targer_step))

        if self.cnt % KeyboardAction.FORWARD_STEP_1.__len__() == 0:
            self.spot_type = "B" if self.spot_type == "A" else "A"
    ...
    def make_unity_env_reset(self):
        ...
        try:
            ...
            # make spot to the initial pose of the first step
            for i in range(KeyboardAction.FORWARD_STEP_1.__len__()):                
                self.__node.publish_spot_actions(KeyboardAction.FORWARD_STEP_1[i])
                time.sleep(self.step_timestep)
            self.spot_type = "A"
            return True
